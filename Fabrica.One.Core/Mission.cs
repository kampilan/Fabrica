...
public class Mission
{
    ...
    public Mission( IPlan plan, IApplianceLoader appLoader, IApplianceInstaller appInstaller, IApplianceFactory appFactory )
    {
    ...
    private CancellationTokenSource MustStopSource { get; } = new ();
    private CancellationToken MustStopToken { get; set; } = new (true);
    private Task? RunTask { get; set; }

    private Result RunResult { get; } = new ();
    ...
    private void _run( IPlan plan )
    {
        ...
        while (!MustStopToken.IsCancellationRequested)
        {
            ...
            foreach (var mgr in Appliances.Where(a => a is {HasStarted: true, HasStopped: true}))
            {
