...
public class GenerateFromKeysHandler: AbstractRequestHandler<GenerateFromKeysRequest,Stream>
{


    public GenerateFromKeysHandler( ITemplateSourceProvider provider, IReadOnlyDictionary<string, IMergeFieldFormatter> formatters )
    {
    ...
    protected override async Task<Stream> Perform(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var key in Request.TemplateKeys)
        {

            await using (var content = await Provider.GetContent(key))
            {

                var document = DocumentModel.Load( content, LoadOptions.DocxDefault );
