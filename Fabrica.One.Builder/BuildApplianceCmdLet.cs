...
namespace Fabrica.One.Builder
{
    ...
    public class BuildApplianceCmdLet: Cmdlet
    {
        ...
        public string HashTemplate { get; set; } = "appliances/{0}/{0}-{1}.sha";
        ...
        private async Task _uploadToS3( IAmazonS3 client, string key, string hashKey, string package )
        {
            ...
            await using( var content = new FileStream(package, FileMode.Open, FileAccess.Read) )
            {
                ...
                var hashB64  = Convert.ToBase64String(bytes);
                var hashHex = BitConverter.ToString(bytes).Replace("-", "").ToLowerInvariant();


                var builder = new StringBuilder();
                builder.AppendLine( "*****************************************************************" );
                builder.AppendFormat("SHA256 hash of ({0})", key);
                builder.AppendLine();
                builder.AppendLine( "*****************************************************************" );
                builder.AppendLine();
                builder.AppendFormat("Base64: ({0})", hashB64);
                builder.AppendLine();
                builder.AppendFormat("Hex:    ({0})", hashHex );
